...
FILE_ABOUT: dict[str, str] = {
    "test_prompts.py": "сборка промптов: режимы встречи, право промолчать, разбор ответа",
    "test_hints.py": "движок подсказок: триггер, дедуп, бэкофф, SKIP, кнопка «подсказать сейчас»",
    "test_llm_provider.py": "клиент OpenAI-совместимого API, роутер local↔api, бюджет контекста",
    "test_ollama_client.py": "клиент локальной Ollama: запросы и обработка ошибок",
    "test_summary_format.py": "форматирование транскрипта для протокола и лимиты длины",
