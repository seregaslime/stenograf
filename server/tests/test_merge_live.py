...
На PostgreSQL реплика, приписанная удалённому спикеру, падает с ошибкой
внешнего ключа — посреди идущей встречи. Пока база была SQLite, опасность была
тихой: ключи там не проверяются, и такая реплика молча записывалась ссылкой в
никуда, не находясь потом ни у одного участника. Уведомление нужно в обоих
случаях, но цена его отсутствия выросла с «потерялась реплика» до «оборвалась
запись встречи».
"""
import pytest
