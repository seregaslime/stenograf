...
class Settings(BaseSettings):
    ...
    # Пауза, после которой фраза считается законченной. При смене говорящего
    # пауза обычно 200–400 мс — и прежние 300 пропускали всю нижнюю половину:
    # двое, говорящие с паузой 200 мс, склеивались в одну реплику, и её целиком
    # забирал тот, кто говорил дольше («один спикер съедает другого»).
    #
    # Замер scripts/eval_split.py (синтез, двое встык → нужно 2 реплики):
    #   пауза 200 мс: при 300 — 1 реплика, при 200 — 2;
    ...
    # Паузу около нуля не режет ни один порог: это лечится разрезом по смене
    # голоса, а не по тишине.
