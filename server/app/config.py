...
class Settings(BaseSettings):
    ...
    # Пауза, после которой фраза считается законченной. Двое, говорящие почти
    # встык, склеиваются в одну реплику, и её целиком забирает тот, кто говорил
    # дольше («один спикер съедает другого»).
    #
    # VAD режет не на пороге, а примерно на сотню миллисекунд позже — у него
    # своё запаздывание. Замер scripts/eval_split.py (синтез, настоящая пауза
    # между голосами, нужно 2 реплики):
    #   при 300 режется с 400 мс тишины, при 200 — с 300 мс;
    #   монолог (нужен 1 кусок): при 200 — 1, при 150 — 4.
    # То есть 300 → 200 режет паузы 300–400 мс, которые раньше склеивались.
    # Ниже 200 опускать нельзя: монолог рвётся на куски, а короткий кусок даёт
    # худший отпечаток голоса — узнавание становится хуже, а не лучше.
    # Паузу короче ~300 мс не режет ни один разумный порог: это лечится
    # разрезом по смене голоса, а не по тишине.
