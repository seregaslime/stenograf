...
class Settings(BaseSettings):
    ...
    # Пауза между запросами, когда длинная встреча суммируется по фрагментам.
    # Лимит считается за минуту, поэтому фрагменты подряд упрутся в него так же,
    # как один большой запрос. 60 с — это ровно окно, за которое он обновляется.
    llm_api_rate_pause_s: float = 60.0
