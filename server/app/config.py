...
class Settings(BaseSettings):
    ...
    #
    # 16.09.2026 порог опускали до 200, чтобы двое, говорящие почти встык, не
    # склеивались в одну реплику. Замер на синтезе это поддержал — синтетический
    # монолог при 200 не рвался. Живая запись встречи показала обратное: в речи
    # человека есть запинки внутри фразы («Э-э, у меня… проблема») как раз на
    # 200–300 мс, и 200 их режет. Замер той же записи, канал подкаста:
    #   при 300 — 50 реплик, медиана 5.1 с, короче двух секунд 8;
    #   при 200 — 65 реплик, медиана 3.8 с, короче двух секунд 19.
    # Короткий кусок даёт худший отпечаток голоса, то есть узнавание страдает.
    # Синтез паузу-запинку не содержит — поэтому и не предупредил.
    #
    # VAD режет не на пороге, а примерно на сотню миллисекунд позже: при 300
    # граница проходит около 400 мс настоящей тишины (scripts/eval_split.py).
    # Двоих, говорящих встык быстрее, порог тишины не разведёт ни при каком
    # значении — это лечится разрезом по смене голоса, а не по тишине.
