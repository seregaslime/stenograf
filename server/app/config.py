...
class Settings(BaseSettings):
    ...
    # Реплика короче этого (речь без запаса VAD), чей голос никого не напомнил,
    # остаётся «Неизвестной», а не заводит нового спикера. На секунде звука
    # отпечаток гуляет так, что «Ха-ха» и «Ну зачем?» знакомого человека не
    # узнаются, и каждая такая реплика плодила профиль. Прогон записи подкаста
    # через конвейер (177 реплик, два голоса): без правила лишних профилей 5,
    # все с реплик до 1.2 с речи; при 1.0 — 2; при 1.5 и 2.0 — 0. Взято 2.0 ради
    # запаса: «Неизвестных» 9, цена — второй голос заведён со второй своей
    # реплики (1.7 с), а не с первой. Реплики знакомых голосов порог не трогает.
    speaker_new_min_s: float = 2.0
    ...
    # Смена говорящего внутри реплики (app/diarization/turns.py): голос слева и
    # справа от точки похожи меньше этого — реплика режется. Нужен для двоих,
    # говорящих встык в одном канале: паузы между ними 80–280 мс, VAD их не
    # видит, и реплику целиком забирал тот, кто говорил дольше.
    # Замер scripts/eval_turns.py на записи подкаста (33 реплики не короче 2.5 с):
    #   реплики с двумя голосами — самая похожая 0.198, разрезаны все 10;
    #   монологи — самый непохожий 0.281, ложных разрезов 0 из 23.
    # Порог посередине, запас ±0.04. Одна запись, голоса мужской и женский —
    # два похожих голоса разводить труднее, перемерить на следующей записи.
    speaker_turn_threshold: float = 0.24
