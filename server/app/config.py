...
class Settings(BaseSettings):
    ...
    # Эмбеддинги считает та же Ollama (отдельная модель, не разговорная).
    # bge-m3: 1.2 ГБ, 100+ языков, вход 8k токенов. Живёт в памяти только на
    # время индексации и запроса — дальше её выгружает llm_keep_alive.
    #
    # Вдвое более лёгкая paraphrase-multilingual (562 МБ) на эталоне
    # (scripts/eval_search.py) показала ровно то же качество, но у неё вход
    # 128 токенов, и длинный кусок она молча обрезает: с ключевой фразой в
    # конце куска на 1313 символов близость к запросу упала с 0.639 до 0.187,
    # у bge-m3 — с 0.728 до 0.579. Замер виден только на длинных кусках,
    # поэтому эталон его и не поймал.
    search_embed_model: str = "bge-m3"
