...
class OllamaClient:
    ...
    async def generate(
        ...
        # Режим рассуждений оставляем включённым: у qwen3 «мысли» уходят в отдельное
        # поле ответа, а финальный текст чистый. Выключение think приводит к
        # рассуждениям вслух прямо в ответе. Поэтому и num_predict не ограничиваем —
        # иначе весь лимит уйдёт на размышления и ответ окажется пустым.
        payload: dict = {
            "model": model,
            "prompt": prompt,
            "stream": False,
