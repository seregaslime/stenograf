...
# Ollama здесь для сценария «всё на одном ноутбуке»: к ней обращается само
# приложение по адресу из настроек, сервер языковых моделей не знает. Стоит
# модель на другой машине — эта часть просто ничего не найдёт и не помешает.
#   ./start.sh
# Сервер пишет лог в server.log. Остановить сервер: pkill -f "uvicorn app.main"
cd "$(dirname "$0")"

# 1. Ollama (её зовёт приложение — за резюме, подсказками и эмбеддингами)
