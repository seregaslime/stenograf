...
describe("документы на экране", () => {
  ...
  it("отказ сервера при загрузке показывается", async () => {
    отказЗагрузки = new Error("Пока принимаются файлы .txt, .md и .docx.");
    await открыть();
    await выбрать(new File(["%PDF"], "договор.pdf"));
    expect(container.querySelector(".banner.error")?.textContent).toBe("Пока принимаются файлы .txt, .md и .docx.");
  });
