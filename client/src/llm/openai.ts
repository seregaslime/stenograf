 ...
 * это один limit токенов в минуту на всех сразу.
 *
...
export interface OpenAiOptions {
  baseUrl: string;
  apiKey?: string;
  /** Минимальный context модели, чтобы она попала в список (окно подсказок ~16k). */
  minContextTokens?: number;
}

/** Кусок потокового ответа: text ответа и ход мыслей идут раздельно. */
...
export class OpenAiClient {
  ...
  /**
   * Ждёт, пока провайдер восстановит limit под запрос такого размера. Спрашиваем
   ...
   * text→text и слишком маленький context; если полей нет — не прячем,
   * судить не по чему.
   */
   ...
   * тоже suitable, заголовки приходят и с ним.
   */
